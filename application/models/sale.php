...
class Sale extends CI_Model {
    ...
    /**
     * Added By @copyright (c) 2014, Eranda Bandaranaike
     */
    public function save_commition($sale_id, $customer_id, $ammount) {
        $data = array(
            "sale_id" => $sale_id,
    ...
    public function save_check_due_date($sale_id, $save_check_due_date) {
        $data = array(
            "inserted_date" => date("Y-m-d"),
            "sale_id" => $sale_id,
            "banking_date" => date("Y-m-d", strtotime($save_check_due_date))
        );
